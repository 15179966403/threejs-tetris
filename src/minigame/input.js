 ...
 *   浮层状态：点任意处 = 主操作（开始/继续/再来一局）；「选择」键仍可切换布局
 *
...
export class Input {
  ...
  _onStart(e) {
    ...
    // 浮层状态（ready / paused / gameover）：抬起时触发主操作；「选择」键仍可用
    if (st !== 'playing' && st !== 'clearing') {
      if (this.ui.hitControl(x, y) === 'select') {
        this.a.swap();
        return;
      }
    ...
    // 已有手指按住十字键：第二根手指只响应 开始/选择
    if (this.dpadTouch) {
      if (hit === 'start') this.a.pause();
      else if (hit === 'select') this.a.swap();
      return;
    }
