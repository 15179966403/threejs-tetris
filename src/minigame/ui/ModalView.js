/**
 * 弹窗系统组件：就绪 (ready)、暂停 (paused)、结算 (gameover) 以及全局设置 (settings) 弹窗。
 */
...
export class ModalView {
  ...
  _drawSettingsModal(c, W, H, side, settings, controlsOut) {
    ...
    const vibeBtnW = 80;
    const vibeBtnH = 28;
    const vibeBtnX = px + pw - 24 - vibeBtnW;
    controlsOut.settingVibe = { x: vibeBtnX, y: vibeY, w: vibeBtnW, h: vibeBtnH };

    const vibeOn = settings ? settings.vibrateEnabled !== false : true;
    drawRoundRect(c, vibeBtnX, vibeY, vibeBtnW, vibeBtnH, vibeBtnH / 2);
    ...
    c.textAlign = 'center';
    c.fillText(vibeOn ? '📳 已开启' : '关闭', vibeBtnX + vibeBtnW / 2, vibeY + vibeBtnH / 2 + 3.5);
